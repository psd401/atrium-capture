...
Production publication uses Atrium's documented OAuth, immutable content-asset, idempotent content/version/publication, and concurrency contracts. Approved public client UUIDs are bundled and are not credentials. The authorization redirect and first-party login now work, but browser token exchange fails closed until Atrium explicitly permits the exact stable `chrome-extension://jldnpmcpimhabiphcglkbgmbffpoocpo` origin for that exact browser client; no wildcard origin or client-side proxy is acceptable. Asset initiation itself is not idempotent; after an ambiguous reservation response, the client recovers matching rows or waits for expiry rather than racing another reservation. ADR 0006 records both external boundaries. No substitute screenshot host is permitted.
