...
The synthetic example at `apps/browser-extension/policy/example-managed-policy.json` contains no district identifiers and is safe for local validation. Administrators must substitute approved origins and a documented Atrium collection UUID.
...
4. Open Support diagnostics and confirm `Policy: valid`, the expected retention modes, and the intended origin counts.
5. Test one allowed origin and one denied origin with synthetic content before authorizing real use.
