...
- Object creation requires `visibility: private`. A ready private draft and its
  private authoring link are terminal for the default operation. Internal
  publication is a separate explicit command with its own idempotency key.
