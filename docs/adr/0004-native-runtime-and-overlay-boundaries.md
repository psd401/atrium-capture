...
- Explicit quick captures append to the current reviewable guide and persist through the same receipt envelope. Adding a capture or manual step to a prepared-but-unpublished guide returns it to privacy review. The guide title may change without invalidating approved pixels, but every content/title edit locks once a durable Atrium publish job exists so retries preserve the original request and idempotency hash.
