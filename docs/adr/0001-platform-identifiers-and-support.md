...
- Browser OAuth redirect: `https://jldnpmcpimhabiphcglkbgmbffpoocpo.chromiumapp.org/atrium-oauth`.
- macOS bundle identifier: `org.psd401.AtriumCapture`.
- Minimum macOS version: macOS 14 Sonoma.
- Native OAuth callback scheme requested from Atrium: `org.psd401.AtriumCapture:/oauth/callback`, subject to Atrium RFC 8252 support.
...
The extension ID and OAuth redirects must not change after client registration. macOS 13 and older are outside the initial native support matrix. Live OAuth remains capability-gated until Atrium registers these redirect URIs and provides production-capable token signing and validation.
