...
- The official `swift:6.0-bookworm` suite executes 28 tests across shared fixtures, recorder persistence/restart, duplicate merging, ordering, secure-field rejection, generic input intent, exact bridge validation, serialized screenshots, mandatory sensitive-step redaction, durable publishing and terminal-outbox restoration, mixed-scale display geometry, pixel sampling, bounded pins, and clipboard retention.
