...
- The Swift suite executes 41 tests on macOS across shared fixtures, recorder persistence/restart, duplicate merging, ordering, secure-field rejection, generic input intent, exact bridge validation, serialized screenshots, mandatory sensitive-step redaction, durable publishing, file-backed outbox restart/raw-byte cleanup, terminal-outbox restoration, mixed-scale display geometry, pixel sampling, bounded pins, clipboard retention, production gateway configuration, sequential TCC prompting, interactive annotation placement, every renderer tool, four arrow directions, and the off-main AuthenticationServices callback.
